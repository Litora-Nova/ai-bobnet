...
> built and tested. The full serialized event spine, provider-wide reference monitor, Gate/Grant/Effect
...
>
> The delivery and memory contracts define an intermediate legacy-journal commit protocol. A contract
> definition is not an implementation-status claim. The protocol is a prelude to §6, not its
> implementation: it does not add sequence numbers, event envelopes, framing, integrity markers, a
> project `main` stream, cursors, projectors, gap detection, or torn-tail recovery.
